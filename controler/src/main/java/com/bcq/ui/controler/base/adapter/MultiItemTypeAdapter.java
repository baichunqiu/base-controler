...
import com.bcq.ui.controler.base.adapter.entry.ItemDelegate;
import com.bcq.ui.controler.base.adapter.entry.ItemDelegateManager;
...
public class MultiItemTypeAdapter<T> extends AbsBaseAdapter<T> {
    ...
    public int getItemViewType(int position) {
        if (useItemDelegateManager()) {
            int viewType = mItemDelegateManager.getItemViewType(mDatas.get(position), position);
            return viewType;
        }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ItemDelegate itemDelegate = mItemDelegateManager.getItemDelegate(mDatas.get(position), position);
    ...
    @Override
    public int getCount() {
        return null == mDatas ? -1 : mDatas.size();
    }
